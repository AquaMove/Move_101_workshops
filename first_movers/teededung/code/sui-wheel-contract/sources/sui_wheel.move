...
entry fun spin_wheel(wheel: &mut Wheel, random: &Random, clock: &Clock, ctx: &mut TxContext) {
    ...
    // Check pool sufficiency before spin
    let mut total_prizes: u64 = 0;
    let mut i = 0;
    while (i < vector::length(&wheel.prize_amounts)) {
        total_prizes = total_prizes + *vector::borrow(&wheel.prize_amounts, i);
    ...
    if (num_entries == 1) {
        // Auto-assign if only one entry left (e.g., when unique entries == prizes and no duplicates)
        ...
        winner_addr = vector::swap_remove(&mut wheel.remaining_entries, rand_index);
        // Remove all other entries of this winner_addr
        let mut i = 0;
        while (i < vector::length(&wheel.remaining_entries)) {
            if (*vector::borrow(&wheel.remaining_entries, i) == winner_addr) {
                vector::swap_remove(&mut wheel.remaining_entries, i);
            } else {
                i = i + 1;
            };
        };
